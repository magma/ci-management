...
#Monit setup script for magmacore jnlp agent connections

test_firewall() {
jenkins_hostname="${jenkins_hostname:-jenkins-new.magmacore.org}"
...
usage: $0 [OPTIONS]
 ...
 -n  set agent name
 -s  set secret key
 -l  set host, default is jenkins-new.magmacore.org
 -b  set flag that this connection is for sandbox; default is jenkins-new.magmacore.org/sandbox
...
if [[ ${SANDBOX} == true ]]; then
    jenkins_hostname="${jenkins_hostname:-jenkins-new.magmacore.org/sandbox}"
else
   jenkins_hostname="${jenkins_hostname:-jenkins-new.magmacore.org}"
fi
